...
## 14) Spielwelt-Struktur

### Aufbau einer Zone
- Zonen bestehen aus einer linearen Abfolge von fixen Stationen (Option C).
- Der Spieler navigiert nicht frei, sondern schreitet Station fuer Station voran.
- Jede Station kann Gegner, Dialoge, Ereignisse oder eine Kombination enthalten.

### Was ist ein Run?
- Ein Run umfasst eine komplette Zone von der ersten Station bis zum Boss.
- Der Run endet mit dem Boss-Kampf oder dem Tod des Spielers.

### Idle vs. aktive Spielwelt
- Die Spielwelt ist nicht persistent im Sinne von Gegner-Respawn.
- Idle ist ein paralleler, unabhaengiger Stream, der nur aktiv ist, wenn der Spieler nicht aktiv spielt.
- Aktive Spielwelt und Idle-Stream laufen nie gleichzeitig.

### Kampagnen-Konfiguration
- Kampagnenabschnitte sind frei konfigurierbar — kein hartcodierter Aufbau.
- Pro Abschnitt kann individuell festgelegt werden:
  - Anzahl und Art der Stationen
  - Gegner pro Station
  - Zeitpunkt und Art des Boss-Encounters
  - Dialoge und Handlungsmomente an beliebigen Stationen
- Ziel: Die Kampagne kann inhaltlich und dramaturgisch flexibel gestaltet werden, ohne technische Anpassungen am System.
- Ein Editor oder Konfigurationsformat fuer Abschnitte ist Teil des Prototyp-Scopes (Format noch zu definieren).
